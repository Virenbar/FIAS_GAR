...
-- Description:	Создает выражение для полнотекстового поиска испольхуя NEAR
-- =============================================
CREATE FUNCTION [dbo].[SUF_ExpressionNEAR](
	@Address VARCHAR(500))
RETURNS VARCHAR(500)
AS
BEGIN
	...
	SET @Result = ISNULL('"' + REPLACE(@Prefix, ' ', '" ~ "') + '"' + ' ~ ', '') + '"' + REPLACE(@Address, ' ', '*" ~ "') + '"'
	RETURN @Result
END